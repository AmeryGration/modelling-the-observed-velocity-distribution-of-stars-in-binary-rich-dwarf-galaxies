...
"""Create binary population of ZAMS stars with canonical IMF"""
...
def create_binary_population(n_binary=1_000_000):
    # Sample primary mass
    rv_mass = sp.stats.truncpareto(1.3, (40. - 0.)/0.8, scale=0.8)
    ...
    data_dir = "moe2017_canonical/data/"
    try:
        pathlib.Path(data_dir).mkdir(parents=True)
    except FileExistsError:
        pass
    df.to_csv(data_dir + "binary_properties.csv", index=False)
